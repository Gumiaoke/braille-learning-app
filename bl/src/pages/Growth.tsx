  ...
  return (
    <div className="p-6 space-y-8">
      <header className="flex justify-between items-center">
        ...
        <button className="p-2 btn-glass rounded-xl text-gray-400">
          <Share2 size={20} />
        </button>
      </header>
      ...
      <section>
        <div className="flex justify-between items-center mb-3">
          <h2 className="text-lg font-black text-gray-800">🌱 今日成长计划</h2>
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-full border-[3px] border-gray-200 border-t-brand-blue animate-spin" />
            <span className="text-xs font-bold text-gray-400">60%</span>
          </div>
        </div>
        <div className="space-y-2">
          {[
            { title: '拼音课程 · 第三课', sub: '声母与韵母练习', badge: '进行中', badgeColor: 'bg-brand-blue/15 text-blue-600', check: '▶', checkColor: 'border-brand-blue text-brand-blue' },
            { title: '阅读故事 · 10分钟', sub: '《点点历险记》', badge: '已完成', badgeColor: 'bg-mint/25 text-green-700', check: '✓', checkColor: 'bg-mint border-mint text-white' },
            { title: '练习题 · 两组', sub: '数学与逻辑思维', badge: '未开始', badgeColor: 'bg-gray-100 text-gray-400', check: '○', checkColor: 'border-gray-300 text-gray-300' },
          ].map((t, i) => (
            <div key={i} className="bg-white rounded-2xl p-3.5 flex items-center gap-3 shadow-sm border border-gray-100">
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold border-2 flex-shrink-0 ${t.checkColor}`}>
                {t.check}
              </div>
              ...
              <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full flex-shrink-0 ${t.badgeColor}`}>{t.badge}</span>
            </div>
          ))}
        </div>
      </section>
